...
public class PercentageColumn extends AbstractColumn {
	...
	public String getTextForExpression(DJGroup group) {
		return "Double.valueOf((" + getPercentageColumn().getTextForExpression() + ").doubleValue() / $V{" + getReportName() + "_" + getGroupVariableName(group) + "}.doubleValue())";
	}	
	...
	public String getTextForExpression(DJGroup group, DJGroup childGroup, String type) {
		return "Double.valueOf( $V{" + getReportName() + "_" +  getGroupVariableName(childGroup) + "}.doubleValue() / $V{" + getReportName() + "_" + getGroupVariableName(type,group.getColumnToGroupBy().getColumnProperty().getProperty()) + "}.doubleValue())";
	}
