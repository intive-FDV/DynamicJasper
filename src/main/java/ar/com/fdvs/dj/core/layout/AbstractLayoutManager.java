...
public abstract class AbstractLayoutManager implements LayoutManager {
	...
	/**
	 * Holds the original groups binded to a column. Needed for later reference List<JRDesignGroup>
	 */
	...
	protected void applyWaterMark() {
		...
		BufferedImage watermark = WaterMarkRenderer.rotateText(djWaterMark.getText(), font2.toAwtFont(), printableWidth * multiplier, printableHeight * multiplier, djWaterMark.getAngle(),
				djWaterMark.getTextColor());
		try {
	...
	protected void transformDetailBand() {
		...
		// TODO: With this new way, we can use template content as it comes, and add a new band for DJ on top or bellow
		// it.
		JRDesignBand detail;
		...
		for (AbstractColumn column : getVisibleColumns()) {
			...
			if (column instanceof BarCodeColumn) {
				...
				// barcodeColumn.isCheckSum() + ", " + barcodeColumn.getApplicationIdentifier() + ","+ column.getWidth()
				...
				imageExp.setText("ar.com.fdvs.dj.core.BarcodeHelper.getBarcodeImage(" + barcodeColumn.getBarcodeType() + ", " + column.getTextForExpression() + ", "
						+ barcodeColumn.isShowText() + ", " + barcodeColumn.isCheckSum() + ", " + applicationIdentifier + ",0,0 )");
	...
	// */
	/*
	 * MOVED INSIDE ExpressionUtils protected JRDesignExpression getExpressionForConditionalStyle(ConditionalStyle
	 * condition, AbstractColumn column) { //String text =
	 * "(("+CustomExpression.class.getName()+")$P{"+paramName+"})."+CustomExpression.EVAL_METHOD_NAME+"("+
	 * textForExpression+")"; String columExpression = column.getTextForExpression();
	 ...
	 * 
	 * String fieldsMap = "(("+DJDefaultScriptlet.class.getName() + ")$P{REPORT_SCRIPTLET}).getCurrentFields()"; String
	 * parametersMap = "(("+DJDefaultScriptlet.class.getName() + ")$P{REPORT_SCRIPTLET}).getCurrentParams()"; String
	 * variablesMap = "(("+DJDefaultScriptlet.class.getName() + ")$P{REPORT_SCRIPTLET}).getCurrentVariables()";
	 * 
	 * String evalMethodParams = fieldsMap +", " + variablesMap + ", " + parametersMap + ", " + columExpression;
	 * 
	 * String text =
	 * "(("+ConditionStyleExpression.class.getName()+")$P{"+condition.getName()+"})."+CustomExpression.EVAL_METHOD_NAME+
	 * "("+evalMethodParams+")"; JRDesignExpression expression = new JRDesignExpression();
	 * expression.setValueClass(Boolean.class); expression.setText(text); return expression; }
	 */
	...
	/**
	 * Given a dj-Style, it is applied to the jasper element. If the style is being used by the first time, it is
	 * registered in the jasper-design, if it is the second time, the one created before is used (cached one)
	 *
	 * @param style
	 * @param designElemen
	 */
	...
	/**
	 * Sets the columns width by reading some report options like the printableArea and useFullPageWidth. columns with
	 * fixedWidth property set in TRUE will not be modified
	 */
	protected void setColumnsFinalWidth() {
		...
		if (report.getOptions().isUseFullPageWidth()) {
			...
			log.debug("printableArea = " + printableArea + ", columnsWidth = " + columnsWidth + ", columnsWidth = " + columnsWidth + ", notRezisableWidth = " + notRezisableWidth
					+ ", factor = " + factor);
	...
	/**
	 * Removes empty space when "fitToContent" is true and real height of object is taller than current bands height,
	...
	protected JRDesignTextField generateTextFieldFromColumn(AbstractColumn col, int height, DJGroup group) {
		...
		if (group != null) {
			...
			/*
			 * Since a group column can share the style with non group columns, if oddRow coloring is enabled, we
			 * modified this shared style to have a colored background on odd rows. We don't want that for group
			...
			/*
			 * if (getReport().getOptions().isPrintBackgroundOnOddRows() && (jrstyle.getConditionalStyles() == null ||
			 * jrstyle.getConditionalStyles().length == 0)) { // No group column so this is a detail text field
			 * JRDesignExpression expression = new JRDesignExpression(); expression.setValueClass(Boolean.class);
			 ...
			 * 
			 * condStyle.setConditionExpression(expression); jrstyle.addConditionalStyle(condStyle); }
			 */
		}
		return textField;
	}
	...
	private void setUpConditionStyles(JRDesignStyle jrstyle, AbstractColumn column) {
		...
		for (ConditionalStyle condition : column.getConditionalStyles()) {
			if (getReport().getOptions().isPrintBackgroundOnOddRows() && Transparency.TRANSPARENT == condition.getStyle().getTransparency()) { // condition
																																				// style
																																				// +
																																				// odd
																																				// row
	...
	protected List<JRDesignVariable> registerChartVariable(DJChart chart) {
		...
		for (Object o : chart.getColumns()) {
			...
			// different types of chart from the very same column (with the same operation also) making the variables
