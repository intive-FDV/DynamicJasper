...
public class DynamicJasperHelper {
	...
	private static void registerEntities(DynamicJasperDesign jd, DynamicReport dr, LayoutManager layoutManager) {
		...
		if (messages == null) {
			try {
				...
				log.warn(e.getMessage() + ", using default (dj-messages)");
				try {
					messages = ResourceBundle.getBundle(DJ_RESOURCE_BUNDLE, Locale.ENGLISH); // this cannot fail because
																								// is included in the DJ
																								// jar
	 ...
	 *            the DynamicReport
	 * @param layoutManager
	 *            the object in charge of doing the layout
	 * @param ds
	 *            The datasource
	 * @param _parameters
		...
		for (Object key : _parameters.keySet()) {
			if (key instanceof String) {
				try {
					...
					// when using an array as subreport-data-source, I must pass the parameter class name like this:
	...
	/**
	 * Compiles the report and applies the layout. <b>generatedParams</b> MUST NOT BE NULL All the key objects from the
	 * generatedParams map that are String, will be registered as parameters of the report.
	 *
	 * @param dr
	 * @param layoutManager
	...
	/**
	 * Performs any needed operation on subreports after they are built like ensuring proper subreport with if
	 * "fitToParentPrintableArea" flag is set to true
	 *
	 * @param dr
	 * @param _parameters
	 * @throws JRException
