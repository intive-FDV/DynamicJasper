...
import ar.com.fdvs.dj.domain.entities.container.*;
...
public class ClassicLayoutManager extends AbstractLayoutManager {
    ...
    protected void generateTitleBand() {
        ...
        if (!getReport().getTitleElements().isEmpty()) {
            ...
            JRDesignExpression printWhenExpression1 = ExpressionUtils.createExpression(EXPRESSION_TRUE_WHEN_FIRST_PAGE, Boolean.class);
            layoutGraphicElementsInBand(band, getReport().getTitleElements(), voffset + 1,
                    printWhenExpression1, null);
        }


    }

    protected void layoutGraphicElementsInBand(JRDesignBand band, List<ElementContainer> elements, int vOffset, JRDesignExpression printWhenExpression, JRGroup evaluationGroup) {
        for (ElementContainer elementContainer : elements) {
            layoutGraphicElementsInBand(band, elementContainer, vOffset, getReport().getOptions().getPrintableWidth(), printWhenExpression, evaluationGroup);
     ...
     */
    protected void layoutGraphicElementsInBand(JRDesignBand band, ElementContainer elementContainer, int vOffset, int parentWidth, JRDesignExpression printWhenExpression, JRGroup evaluationGroup) {
        if (elementContainer == null || elementContainer.getElementsArray().length == 0)
            return;
        ...
        if (availableNonFixed <= 0) {
            throw new LayoutException("No room for elements in band. Fixed width elements width are wider than available print space.");
        }
        ...
        for (GraphicElement nonFixedElement : nonFixedElements) {
            nonFixedElement.setWidth((int) (nonFixedElement.getWidth() * factor));
        ...
        if (remaining > 0) {
            GraphicElement lastNonFixedElement = nonFixedElements.get(nonFixedElements.size() - 1);
            lastNonFixedElement.setWidth(lastNonFixedElement.getWidth() + remaining);
        }
        ...
        jrDesignFrame.setStretchType(StretchTypeEnum.NO_STRETCH);
        jrDesignFrame.setPositionType(PositionTypeEnum.FLOAT);
        jrDesignFrame.setPrintWhenExpression(printWhenExpression);
        jrDesignFrame.setRemoveLineWhenBlank(true);


        for (GraphicElement graphicElement : elementContainer.getElementsArray()) {
            layoutSimpleElement(jrDesignFrame, graphicElement, evaluationGroup);
        }
    ...
    protected void layoutSimpleElement(JRDesignFrame jrDesignFrame, GraphicElement graphicElement, JRGroup evaluationGroup) {
        if (graphicElement instanceof TextElement) {
            TextElement ste = (TextElement) graphicElement;
            JRDesignTextField jrst = new JRDesignTextField();
            jrst.setExpression(ExpressionUtils.createStringExpressionWithText(ste.getText()));
            ...
            jrst.setEvaluationGroup(evaluationGroup);
            if (evaluationGroup != null)
                jrst.setEvaluationTime(EvaluationTimeEnum.GROUP);
            applyStyleToElement(ste.getStyle(), jrst);
            jrDesignFrame.addElement(jrst);

        } else if (graphicElement instanceof CustomExpressionElement) {
            CustomExpressionElement customExpressionElement = (CustomExpressionElement) graphicElement;

            JRDesignTextField jrst = new JRDesignTextField();
            String expName = "expression_for_graphic_element_" + Math.abs(new Random().nextInt());
            JRDesignExpression jrde = ExpressionUtils.createAndRegisterExpression(getDesign(), expName, customExpressionElement.getCustomExpression());

            jrst.setExpression(jrde);
            jrst.setX(customExpressionElement.getPosX());
            jrst.setY(0);
            jrst.setHeight(customExpressionElement.getHeight());
            jrst.setWidth(customExpressionElement.getWidth());
            jrst.setStretchType(StretchTypeEnum.RELATIVE_TO_TALLEST_OBJECT);
            jrst.setPositionType(PositionTypeEnum.FIX_RELATIVE_TO_TOP);
            jrst.setMarkup("styled");
            jrst.setEvaluationGroup(evaluationGroup);
            //if (evaluationGroup != null)
              //  jrst.setEvaluationTime(EvaluationTimeEnum.GROUP);
            applyStyleToElement(customExpressionElement.getStyle(), jrst);

            jrDesignFrame.addElement(jrst);
        } else if (graphicElement instanceof ImageElement) {
            ImageElement imageElement = (ImageElement) graphicElement;
            JRDesignImage jrdi = new JRDesignImage(new JRDesignStyle().getDefaultStyleProvider());

            if (imageElement.getPath() != null) {
                jrdi.setExpression(ExpressionUtils.createExpressionForPath(imageElement.getPath()));
            } else if (imageElement.getInputStream() != null) {
                final String imageParameterName = "imageInputStream_" + Math.abs(new Random().nextInt());
                CustomExpression ie = new CustomExpression() {
                    @Override
                    public Object evaluate(Map fields, Map variables, Map parameters) {
                        InputStream imageIs = (InputStream) parameters.get(imageParameterName + "_is");
                        try {
                            return ImageIO.read(imageIs);
                        } catch (IOException e) {
                            log.error("Could not create Image for Graphic element", e);
                        }
                        return null;
                    }
                ...
                //Register the parameter with the actual InputStream
                LayoutUtils.registerParameterWithValue(getDesign(), imageParameterName + "_is", imageElement.getInputStream());

                //Register the customExpression as a parameter and create the expression
                JRDesignExpression exp = ExpressionUtils.createAndRegisterExpression(getDesign(), imageParameterName, ie);
                jrdi.setExpression(exp);

            } else {
                throw new DJException("Image element must have a path or an inputStream to an image");
            }

            jrdi.setWidth(imageElement.getWidth());
            jrdi.setHeight(imageElement.getHeight());
            jrdi.setScaleImage(ScaleImageEnum.getByValue(imageElement.getScaleMode().getValue()));
            jrdi.setHorizontalAlignment(HorizontalAlignEnum.getByValue(imageElement.getHorizontalAlign().getValue()));
            jrdi.setRemoveLineWhenBlank(true);
            applyStyleToElement(imageElement.getStyle(), jrdi);

            jrDesignFrame.addElement(jrdi);
        }
    }
    ...
    protected void layoutGroups() {
        ...
        for (Object o1 : getReport().getColumnsGroups()) {
            DJGroup djGroup = (DJGroup) o1;
            JRDesignGroup jgroup = getJRGroupFromDJGroup(djGroup);

            jgroup.setStartNewPage(djGroup.getStartInNewPage());
            jgroup.setStartNewColumn(djGroup.getStartInNewColumn());
            jgroup.setReprintHeaderOnEachPage(djGroup.getReprintHeaderOnEachPage());
            ...
            header.setHeight(djGroup.getHeaderHeight());
//			footer.setHeight( getFooterVariableHeight(columnsGroup));
            footer.setHeight(djGroup.getFooterHeight());

            header.setSplitType(LayoutUtils.getSplitTypeFromBoolean(djGroup.isAllowHeaderSplit()));
            footer.setSplitType(LayoutUtils.getSplitTypeFromBoolean(djGroup.isAllowFooterSplit()));
            ...
            if (djGroup.getLayout().isPrintHeaders()) {
                ...
                for (Object o : getVisibleColumns()) {
                    ...
                    if (col.equals(djGroup.getColumnToGroupBy())) {
                        found = true;
                    }
                    ...
                    JRDesignTextField designTextField = createColumnNameTextField(djGroup, col);
...
//				JRDesignBand band = (JRDesignBand)jgroup.getGroupFooter();
                ...
                if (label.getLabelPosition() == LabelPosition.LEFT) {
                    ...
                    x = col.getPosX(); //label starts in the column-to-group-by x position
                    y = getFooterVariableHeight(djGroup);
                    width -= x;
                }
                layoutGroupFooterLabels(djGroup, jgroup, x, y, width, height);
                layoutGroupVariables(djGroup, jgroup, yOffset);
            } else {
                layoutGroupVariables(djGroup, jgroup, 0);
            }

            layoutGroupSubreports(djGroup, jgroup);
            layoutGroupCrosstabs(djGroup, jgroup);
            layoutGroupElements(djGroup, jgroup);
        }
    }

    private void layoutGroupElements(DJGroup djGroup, JRDesignGroup jgroup) {
        //for HEADER
        if (!djGroup.getHeaderElements().isEmpty()) {
            JRDesignSection section = (JRDesignSection) jgroup.getGroupHeaderSection();
            JRDesignBand elementsBand = new JRDesignBand();
            section.addBand(0, elementsBand);
            layoutGraphicElementsInBand(elementsBand, djGroup.getHeaderElements(), 0, null, jgroup);
        }

        if (!djGroup.getFooterElements().isEmpty()) {
            JRDesignSection section = (JRDesignSection) jgroup.getGroupFooterSection();
            JRDesignBand elementsBand = new JRDesignBand();
            section.addBand(elementsBand);
            layoutGraphicElementsInBand(elementsBand, djGroup.getFooterElements(), 0, null, jgroup);
        }
    }
