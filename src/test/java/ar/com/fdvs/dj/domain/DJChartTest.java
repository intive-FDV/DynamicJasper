...
class DJChartTest {
    ...
    class Constants {
        ...
        void chartTypeConstants() {
            // JR 6.21 value: DJChart.*_CHART is JRChart.CHART_TYPE_* (byte constants, 1-indexed
            // in JR 6.21.5), not the 0-indexed ordinal() values used on the JR7 branch.
            assertEquals((byte) 1, DJChart.AREA_CHART);
            assertEquals((byte) 3, DJChart.BAR_CHART);
            assertEquals((byte) 2, DJChart.BAR3D_CHART);
            assertEquals((byte) 7, DJChart.LINE_CHART);
            assertEquals((byte) 9, DJChart.PIE_CHART);
            assertEquals((byte) 8, DJChart.PIE3D_CHART);
            assertEquals((byte) 10, DJChart.SCATTER_CHART);
            assertEquals((byte) 12, DJChart.STACKEDBAR_CHART);
            assertEquals((byte) 11, DJChart.STACKEDBAR3D_CHART);
            assertEquals((byte) 20, DJChart.STACKEDAREA_CHART);
            assertEquals((byte) 16, DJChart.TIMESERIES_CHART);
            assertEquals((byte) 13, DJChart.XYAREA_CHART);
            assertEquals((byte) 14, DJChart.XYBAR_CHART);
            assertEquals((byte) 15, DJChart.XYLINE_CHART);
        }
