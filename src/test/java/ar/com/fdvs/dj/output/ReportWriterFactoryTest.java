...
public class ReportWriterFactoryTest {
    ...
    @Test
    @Ignore("backport: JR 6.21.5's JRAbstractExporter.setParameters(null) leaves the " +
            "exporter's internal parameters map null, so the subsequent " +
            "exporter.setParameter(...) call in ReportWriter's constructor NPEs " +
            "(net.sf.jasperreports.engine.JRAbstractExporter.setParameter). This is a real " +
            "robustness gap in ReportWriterFactory.getReportWriter/ReportWriter when null " +
            "parameters are passed; per the backport plan, main is not touched to make a newly " +
            "ported test pass, so this is disabled instead.")
